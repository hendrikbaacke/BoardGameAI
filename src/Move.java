...
public class Move {
	// first, second, and third marble code that need to be moved (the hexagon
	// in the hashboard contains the marbles of the same code- which can be
	// removed and added quickly)
	public String first;
	public String second;
	public String third;
	public String opponnet;
	public String other;
	...
	public void select(String code) {
		...
		// check whether a marble from the player is selected, then set that as
		// a code and add it to the selection:
		...
		// if the first and second are the same, end selection (you're done) it
		// also checks whether they are adjacent
		// if not adjacent: the second selected marble becomes the first - else
		// just add it to the selection
			...
			if (Board.hashBoard.get(code).marble.playerNumber == playersTurn) {
				...
				} else if (!Board.hashBoard.get(code)
						.adjacent(Board.hashBoard.get(first))) {
		...
		// quite similar to the second selection - checks whether it is adjacent
		// to the second one, if not, then it becomes the first in the selection
		// if this is done, selected becomes true automatically
		else if (third == null && !selected
				&& !Board.hashBoard.get(code).empty) {
			if (Board.hashBoard.get(code).marble.playerNumber == playersTurn) {
				if (first.equals(code) || second.equals(code)) {
					...
					selected = true;
						...
						.adjacent(Board.hashBoard.get(second))
						|| !Board.rows.sameRowThree(Board.hashBoard.get(first),
								Board.hashBoard.get(second),
								Board.hashBoard.get(code))) {
					selectedMarbles.clear();
					coloursBackToNormal();
					first = code;
		...
		// so if all of these are not possible, try if it is possible to move it
		// to the place you want to move it to
		// if it's not, then it will automatically reset
		else {
			if (selected && Board.hashBoard.get(code)
					.adjacent(Board.hashBoard.get(first))) {
				if (!Board.hashBoard.get(code).empty) {
					if (Board.hashBoard
							.get(code).marble.playerNumber == playersTurn) {
						selectedMarbles.clear();
						coloursBackToNormal();
						selected = false;
						first = code;
			...
			} else if (!Board.hashBoard.get(code).empty && Board.hashBoard
					.get(code).marble.playerNumber == playersTurn) {
				selectedMarbles.clear();
				coloursBackToNormal();
				selected = false;
	...
	public void move() {
		// check if valid -> if not, reset, else: perform movement, change
		// player, resetmove
		...
		if (nrSelected == 1) {
			if (validMoveOne()) {
				System.out.println(
						Board.rows.direction(Board.hashBoard.get(first),
								Board.hashBoard.get(moveTo)));
				performMovementOne();
				changePlayer();
				resetMove();
			}
	...
	public void performMovementTwo() {
		...
		if (Board.rows.sideways(Board.hashBoard.get(first),
				Board.hashBoard.get(second), Board.hashBoard.get(moveTo))) {
			moveSideways();
	...
	public void pushOne() {
		...
		Marble on = Board.hashBoard.get(first).marble;
		Marble sec = Board.hashBoard.get("second").marble;
		Marble thi = Board.hashBoard.get(moveTo).marble;
		
		Board.hashBoard.get(first).setFull(sec);
		Board.hashBoard.get(second).setEmpty();
		Board.hashBoard.get(moveTo).setFull(on);
		Board.hashBoard.get(other).setFull(thi);
		
		on.setLocationKey(moveTo);
		on.updateLocation();
		sec.setLocationKey(first);
		sec.updateLocation();
		thi.setLocationKey(other);
		thi.updateLocation();
	...
	public void moveSideways() {
		int direction = Board.rows.direction(Board.hashBoard.get(first),
				Board.hashBoard.get(moveTo));
	...
	public boolean validMoveOne() {
		if (Board.hashBoard.get(first).adjacent(Board.hashBoard.get(moveTo))
				&& Board.hashBoard.get(moveTo).empty) {
			return true;
		} else {
			resetMove();
			return false;
	...
	public boolean validMoveTwo() {
		// if it needs to move sideways and if there are two free space where
		// they are needed, the move is valid
		if (Board.rows.sideways(Board.hashBoard.get(first),
				Board.hashBoard.get(second), Board.hashBoard.get(moveTo))) {
			System.out.println("sideways is true");
			if (Board.rows.twoFree(Board.hashBoard.get(first),
					Board.hashBoard.get(second), Board.hashBoard.get(moveTo))) {
				return true;
			}
		} else if (Board.hashBoard.get(moveTo).empty) {
	...
	public boolean validMoveThree() {
		if (Board.rows.sideways(Board.hashBoard.get(first),
				Board.hashBoard.get(second), Board.hashBoard.get(moveTo))) {
			if (Board.rows.threeFree(Board.hashBoard.get(first),
					Board.hashBoard.get(second), Board.hashBoard.get(third),
					Board.hashBoard.get(moveTo))) {
				return true;
			}
		} else if (Board.hashBoard.get(moveTo).empty) {
	...
	public boolean validPush() {

		// if it needs to move sideways and if there are two free space where
		// they are needed, the move is valid
		if (Board.hashBoard.get(first).marble.playerNumber == Board.hashBoard
				.get(second).marble.playerNumber) {
			System.out.println("push 1 ball of player1 using player2");
			if (Board.hashBoard
					.get(moveTo).marble.playerNumber != Board.hashBoard
							.get(first).marble.playerNumber) {
				if (Board.hashBoard.get(moveTo)
						.adjacent(Board.hashBoard.get(other))
						&& Board.hashBoard.get(other).empty) {
					System.out.println("push is valid");
					return true;
				}
